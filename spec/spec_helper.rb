...
class SpecHelpers
  ...
  def stub_build_methods
    stub_request(:get, Rhohub.resource_url({:app_id => 4})).to_return({:body => [{:id=> 4}].to_json})
    stub_request(:get, Rhohub.resource_url({:app_id => 4, :id => 2})).to_return({:body => [{:id=> 4,:status => 'complete'}].to_json})
    stub_request(:delete, Rhohub.resource_url({:app_id => 4, :id => 2})).to_return({:body => [{:text => true}].to_json})
    stub_request(:post, Rhohub.resource_url({:app_id => 4})).to_return({:body => [{:id => 4, :status => 'pending'}].to_json})
  end
